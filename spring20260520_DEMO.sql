...
CREATE TABLE pmt_like (
    ...
    CONSTRAINT FK_PMT_LIKE_ID       FOREIGN KEY (id)       REFERENCES PROMPT     (id) ON DELETE CASCADE,
    CONSTRAINT FK_PMT_LIKE_USER_NUM FOREIGN KEY (user_num) REFERENCES users (user_num) ON DELETE CASCADE,
...
-- BOARD_CARD
CREATE TABLE "SPRING"."BOARD_CARD" 
(	
    "CARD_SEQ" NUMBER, 
    "CMENU_ID" VARCHAR2(20 BYTE) NOT NULL, 
    "CTITLE" VARCHAR2(200 BYTE) NOT NULL, 
    "CCONTENT" VARCHAR2(1000 BYTE), 
    "CIMAGE_URL" VARCHAR2(500 BYTE) NOT NULL, 
    "CLINK_URL" VARCHAR2(500 BYTE) NOT NULL, 
    "CHIT" NUMBER DEFAULT 0, 
    "CTAG" VARCHAR2(500 BYTE)
);

-- AI_SEARCH_WORDS
CREATE TABLE "SPRING"."AI_SEARCH_WORDS" 
(	
    "WORD_SEQ" NUMBER, 
    "CMENU_ID" VARCHAR2(20 BYTE) NOT NULL, 
    "CTITLE" VARCHAR2(200 BYTE) NOT NULL, 
    "SEARCH_WORD" VARCHAR2(100 BYTE) NOT NULL
);
...
(	
    ...
    "SEARCH_WORD"   VARCHAR2(50 BYTE)  NOT NULL
);

--USER_FAVORITE
CREATE TABLE "SPRING"."USER_FAVORITE" 
(	
    "FAVORITE_ID" NUMBER, 
...
-- 유저추가
INSERT INTO users ( user_num, user_id, password, email, role) 
VALUES ( 1, 'admin', '1234', '1234@1234.com', 'ADMIN');

-- PROMPT 추가
INSERT INTO PROMPT (id, cat_id, user_num, Title, Content)
VALUES ((SELECT NVL(MAX(id), 0) + 1 FROM PROMPT),'MM_PMT', 1, 'sapmle', 'aiedhid');
INSERT INTO PROMPT (id, cat_id, user_num, Title, Content)
VALUES ((SELECT NVL(MAX(id), 0) + 1 FROM PROMPT),'EDU_PMT', 1, 'sapmle', 'aiedhid');
